...
def impact(store: Store, config: TorsorConfig, symbol: str, *, limit: int | None = None) -> dict:
    ...
    `mentions` is the other half of the blast radius and the query this
    architecture uniquely enables: the notes — decisions, learnings, handoffs —
    that name this symbol in backticks. The two halves are independent, so a
    symbol nothing calls can still be the one the team argued about."""
    _log_op(store, "impact", symbol)
    empty = {"symbol": symbol, "callers": [], "count": 0, "truncated": 0,
             "mentions": [], "mentions_count": 0}
    ...
    try:
        ...
        mentions = _mentions(conn, store, symbol, base)
    finally:
        conn.close()
    ...
    kept_mentions, _ = cap_items(mentions, cap)
    return {"symbol": symbol, "callers": kept, "count": len(callers),
            "truncated": len(callers) - len(kept),
            "mentions": kept_mentions, "mentions_count": len(mentions)}


def _mentions(conn, store: Store, symbol: str, base: str) -> list[dict]:
    """Notes naming this symbol, as {path, title, type}. Both the dotted spelling
    and the bare one, because a note may write either."""
    paths: list[str] = []
    for spelling in dict.fromkeys((symbol, base)):
        for path in db.notes_mentioning(conn, spelling):
            if path not in paths:
                paths.append(path)
    rows = db.note_rows(conn, paths)
    out = []
    for path in paths:
